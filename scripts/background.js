...
import { getSpendableUtxos, inscribe } from './helpers/devinals';
import {
  clearSessionStorage,
  getCachedTx,
  getLocalValue,
...
import transaction from 'bitcore-lib-doge/lib/transaction';
...
async function onCreateTransaction({ data = {}, sendResponse } = {}) {
  const amountSatoshi = sb.toSatoshi(data.devAmount);
  const amount = sb.toBitcoin(amountSatoshi);

  console.log(amount, data.senderAddress, data.recipientAddress,data.feeRate,  'amount 2')

  try {
    const response = await mydev.post('/api/v1/createtx1', {
    ...
    console.error("🛑 Debugging Log:", response.data);
...
async function onCreateNFTTransaction({ data = {}, sendResponse } = {}) {
  try {
    const response = await mydev.post(
...
async function onInscribeTransferTransaction({ data = {}, sendResponse } = {}) {
  try {
    ...
    // };
    // const feeData = (await mydev.post('/api/v1/wallet/rpc', smartfeeReq)).data;
    ...
    const inscription = `{"p":"dev-20","op":"transfer","tick":"${data.ticker}","amt":"${data.tokenAmount}"}`;
...
async function onCreateDunesTransaction({ data = {}, sendResponse } = {}) {
  try {
    const response = await mydev.post('/api/v1/transactions/prepare/dune', {
      ...
      try {
        ...
        chrome.offscreen
          ...
          //chrome.notifications.create({
            //type: 'basic',
            //iconUrl: 'default-avatar.png', // Make sure you have an icon in your extension
            //title: 'Transaction Sent!',
            //message: `Tx ID: ${jsonrpcRes}`,
          //});
        ...
        // Cache transaction if it's a DEV20 transaction
        if (data.txType) {
...
async function onSendInscribeTransfer({ data = {}, sendResponse } = {}) {
  try {
    ...
    for await (const signed of data.txs) {
      ...
      const jsonrpcRes = (await mydev.post('/api/v1/wallet/rpc', jsonrpcReq))
        .data;
      await cacheSignedTx(signed);
    ...
    txsCache.push({
      txs: results,
      txType: TRANSACTION_TYPES.DEV20_AVAILABLE_TX,
      tokenAmount: data.tokenAmount,
      timestamp: Date.now(),
      ticker: data.ticker,
...
async function onGetDogecoinevPrice({ sendResponse } = {}) {
  try {
    const response = (await mydev.get('/api/v1/ticker/price'))
      .data;
  ...
  try {
    ...
    const balances = await Promise.all(
      addresses.map(async (address) => {
        const response = (await mydev.get(`api/v1/wallet/address/${address}`))
          .data;
...
async function onGetTransactions({ data, sendResponse } = {}) {
  ...
  try {
    const response = (
      await mydev.get(`/api/v1/wallet/address/${data.address}`, {
        params: {
          page: data.page || 1,
    ...
    //const transactions = txIds.sort((a, b) => b.blockTime - a.blockTime);
...
async function onGetTransactionDetails({ data, sendResponse } = {}) {
  try {
    const transaction = (await mydev.get(`/api/v1/tx/${data.txId}`))
      .data;
...
async function onApproveAvailableDEV20Transaction({
  sendResponse,
  data: { txId, error, originTabId, origin, ticker, tokenAmount },
} = {}) {
  if (txId) {
    chrome.tabs?.sendMessage(originTabId, {
      type: MESSAGE_TYPES.CLIENT_REQUEST_AVAILABLE_DEV20_TRANSACTION_RESPONSE,
      data: {
        txId,
        ticker,
        amount: tokenAmount,
...
async function onApproveDevinalTransaction({
  sendResponse,
  data: { txId, error, originTabId, origin },
} = {}) {
  if (txId) {
    chrome.tabs?.sendMessage(originTabId, {
      type: MESSAGE_TYPES.CLIENT_REQUEST_DEVINAL_TRANSACTION_RESPONSE,
      data: {
        txId,
      },
      origin,
  ...
  Promise.all([getLocalValue(PASSWORD), getLocalValue(WALLET)]).then(
    ([encryptedPass, encryptedWallet]) => {
      ...
      if (authenticated) {
        ...
        // MIGRATE Bitcon WIF to Dogecoinev WIF
      ...
      sendResponse: (transaction) => {
        if (transaction?.confirmations >= TRANSACTION_CONFIRMATIONS) {
          chrome.notifications.onClicked.addListener(async (notificationId) => {
            chrome.tabs.create({
              url: `https://dev-mempool.space/tx/${notificationId}`,
            });
          ...
          chrome.notifications.create(txId, {
            ...
            iconUrl: '../assets/mydev128.png',
            message: `${sb.toBitcoin(transaction.vout[0].value)} DEV sent to ${
...
export const messageHandler = ({ message, data }, sender, sendResponse) => {
  ...
  switch (message) {
    ...
    case MESSAGE_TYPES.DELETE_ADDRESS:
      ...
      break;
    case MESSAGE_TYPES.GET_DOGECOINEV_PRICE:
      onGetDogecoinevPrice({ data, sendResponse, sender });
      break;
    ...
    case MESSAGE_TYPES.CLIENT_REQUEST_CONNECTION_RESPONSE:
      ...
      break;
    case MESSAGE_TYPES.CLIENT_REQUEST_DEVINAL_TRANSACTION_RESPONSE:
      onApproveDevinalTransaction({ data, sendResponse, sender });
      break;
    case MESSAGE_TYPES.CLIENT_REQUEST_TRANSACTION:
    case MESSAGE_TYPES.CLIENT_REQUEST_DEVINAL_TRANSACTION:
    case MESSAGE_TYPES.CLIENT_REQUEST_AVAILABLE_DEV20_TRANSACTION:
    ...
    case MESSAGE_TYPES.CLIENT_REQUEST_PSBT_RESPONSE:
      ...
      break;
    case MESSAGE_TYPES.CLIENT_REQUEST_AVAILABLE_DEV20_TRANSACTION_RESPONSE:
      onApproveAvailableDEV20Transaction({ data, sendResponse, sender });
      break;
