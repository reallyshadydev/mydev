...
export async function getDEV20Inscriptions(address, ticker) {
  const query = (
    await mydev.get(
...
export async function getDEV20Balances(address, ticker) {
  const result = (
    await mydev.get(
      `/api/v1/tokens/dev20/${address}${ticker ? `?ticker=${ticker}` : ''}`
    )
  ).data;
