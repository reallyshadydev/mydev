...
/**
 * Format satoshi values for display (convert to whole dev, can specify max decimals)
 * Also uses the decimal separator that matches user's locale
 * @param {string} value
 * @param {integer} [maxDecimals]
 * @returns {string}
 */
export function formatSatoshisAsDev(value, maxDecimals) {
  if (value >= 1) {
    const newValue = sb.toBitcoin(Math.floor(value));
    return formatDev(newValue, maxDecimals);
  } else {
    return formatDev(value / 1e8, 10);
  }
}
...
export function formatDev(value, maxDecimals, useGrouping) {
  let newValue = value;
  const opts = {};
...
// (Expects value to have been formatted via formatSatoshisAsDev/sanitizeDevInput)
