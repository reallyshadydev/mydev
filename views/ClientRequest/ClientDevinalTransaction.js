...
export function ClientDevinalTransaction({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const devinal = await getInscriptionsUtxo(connectedClient?.address, {
        txid,
        vout,
      });

      const inscription = devinal?.inscriptions?.find(
        (i) => i.offset === offset
      );

      if (!devinal || !inscription) {
        handleResponse({
  ...
  return (
    <>
      ...
      <Text fontSize='3xl' fontWeight='semibold' mt='-6px'>
        Dev{transaction.amount}
      </Text>
      <Text fontSize='13px' fontWeight='semibold' pt='6px'>
        Network fee Dev{transaction.fee}
      </Text>
    ...
    sendMessage(
      {
        ...
        data: {
          ...
          txType: TRANSACTION_TYPES.DEVINAL_TX,
        },
      },
      (txId) => {
        setLoading(false);
          ...
          <AlertDialog.Body>
            ...
            <VStack alignItems='center'>
              <Text>
                ...
                <Text fontWeight='bold'>Dev{devAmount}</Text> to{' '}
              </Text>
