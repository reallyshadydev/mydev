 ...
 /* const {
  ...
  const {
    ...
    !walletAddress
      ? null
      : (pageIndex, prevData) => {
          const key = getTransactionsKey(pageIndex, prevData, walletAddress);
          //console.log("getTransactionsKey Output:", key);
          return key;
        },
    getTransactions,
    {
  ...
  const fetchNFTs = useCallback(
    async ({ currentNFTs = [], cursor } = {}) => {
      setNFTsLoading(true);
      try {
        const res = (await mydev.get(`/api/v1/tokens/collections/${walletAddress}`)).data;
  ...
  const fetchTokens = useCallback(
    async ({ cursor, currentTokens = [] } = {}) => {
      ...
      try {
        const dev20res = (await mydev.get(`/api/v1/tokens/dev20/${walletAddress}`)).data;
        const dunes20res = (await mydev.get(`/api/v1/tokens/dunes/${walletAddress}`)).data;
        ...
        setTokensTotal(dev20res.total + dunes20res.total);
        // Don't increment page on initial fetch, where cursor is undefined
        if (typeof cursor === 'number') {
