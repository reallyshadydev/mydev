...
export const NFTComponent = ({ nft, index, onPress, selected }) => {
  ...
  const pendingDevinalTxs = useCachedInscriptionTxs({
    filterPending: true,
  }).filter(
    (tx) =>
      tx.txType === TRANSACTION_TYPES.DEVINAL_TX ||
      tx.txType === TRANSACTION_TYPES.DEV20_SEND_INSCRIPTION_TX
  );

  const isNFTPending = pendingDevinalTxs?.find(
