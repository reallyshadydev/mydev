...
import { formatSatoshisAsDev } from '../../../utils/formatters';
...
export const TransactionModal = ({
  ...
  useEffect(() => {
    (async () => {
      if (isOpen) {
        const tx = (await mydev.get(`/api/v1/tx/${id}`)).data;
        setConf(confirmations);
        ...
        <Modal.Body alignItems='center' pt='54px' pb='36px'>
          <VStack w='100%' alignItems='center'>
            ...
            <Text
              ...
              pb='12px'
            >
              DEV-{formatSatoshisAsDev(amount, 3)}
            </Text>
            ...
            <Box pt='32px'>
              <BigButton
                onPress={() => window.open(`https://dev-mempool.space/tx/${id}`)}
                variant='secondary'
                px='28px'
              >
                View on dev-mempool.space <FiArrowUpRight />
              </BigButton>
            </Box>
          </VStack>
        </Modal.Body>
